...
当建议或引入新的第三方库 (非 Python 内置库, 非 ROS 原生包等) 时, 必须严格执行思维链:

1.  必要性阐述: 简述为何标准库或当前技术栈不足以满足需求.
2.  确认该库是否是当前的最佳实践.
3.  对比分析:
    - 至少列出一个替代选项 (例如: 一个"更重但更普适"的库, 或一个"更新/性能更强"的库).
    - 解释为何在此场景下选择当前库而非替代项的各自优劣对比. 注意当前场景不仅指当前对话的需求, 还应考虑设计文档中提及的未来需求.
4. 确认引入后, 如果是较常更新接口, 需要先获取所需部分的在线文档确认最新接口使用方式
## 调试

- 每次出现问题, 如果是需要执行命令定位的问题, 先给我你的测试方案, 包括命令和预期输出, 请求我批量确认后再执行
...
- 当引入新的命令行工具/库/API时, 需要先实现最小可运行代码/命令行命令, 以验证环境兼容性及正确使用方式
...
- 我提出"更新文档"后, 检查以下项, 对应更新README.md, 否则不需更新文档
  - 架构/关键数据流/关键事件传递更新
  ...
  - 需维护UI交互逻辑
  - 需维护程序配置, 构建, 启动方式
  - 需有一个一级标题是开发 (对于中文文档项目)/Development (对于英语文档项目), 其中列出观测点, 即作为调试关键的变量或输出并给出查看方式
- 结构化输出: 尽量避免大段文字, 利用markdown的样式 (有序/无序列表, 行内代码, 标题, 引用) 结构化文段
- 克制使用加粗, 禁止使用斜体
- 回答问题时, 遵循`结论` -> `风险` -> `原理/依据` -> `步骤` -> `验证` 的逻辑流, 根据情况每步可选
- 语言风格
